// ==UserScript==
// @name        6V活动评分器
// @namespace   neu6activityratehelper
// @author      xingxing
// @grant       unsafeWindow
// @grant       GM_getValue
// @grant       GM_setValue
...
// @updateURL   https://github.com/harleybai/PT-help/raw/master/docs/js/NEU6%20-%20Activity.user.js
// @downloadURL https://github.com/harleybai/PT-help/raw/master/docs/js/NEU6%20-%20Activity.user.js
...
(function () {
	...
	jq('#hdsettings').click(function () {
		let ques_num = GM_getValue('ques_num') ? GM_getValue('ques_num') : 9;
		let hdhtml = '<h3 class="flb"><em id="return_reply">\u6d3b\u52a8\u8bbe\u7f6e</em><span><a href="javascript:;" class="flbc" onclick="hideWindow(\'hdst\')" title="\u5173\u95ed">\u5173\u95ed</a></span></h3><div id="hdsettingdialog" style="width:580px;height:410px;">' +
		...
		let hd_answer1 = GM_getValue('hd_answer');
		...
		jq('#save_open').click(function () {
			let link = "http://bt.neu6.edu.cn/thread-" + jq('#hd_id').val().trim() + "-1-1.html";
			jq('#submitset').click();
			window.open(link);
		});
	...
	function activity_showconfig() {
		let schtml = '<h3 class="flb"><em id="return_reply">\u6d3b\u52a8\u914d\u7f6e</em><span><a href="javascript:;" class="flbc" onclick="hideWindow(\'hdsc\')" title="\u5173\u95ed">\u5173\u95ed</a></span></h3><div id="hdsettingdialog" style="width:540px;height:250px;">' +
		...
		for (let i = 1; i <= activity_quesnum; i++)
	...
	function activity_rate() {
		if (location.href.match(/thread-|tid=/)) {
			let id_match = location.href.match(/(thread-|tid=)(\d+)/);
			let id = id_match ? id_match[2] : 0;
			if (activity_id == id) {
				...
				jq('div#postlist table.plhin').each(function () {
					let content = jq(this).find('div.t_fsz').text();
	...
	function get_score(content, callback) {
		let answer_arr = content.match(/第\d+题[^(第\d+题)]*/g);
		let all_num = Object.keys(activity_answer).length;
		let correct_num = 0;
		correctIndex = "[";
		if (answer_arr && answer_arr.length <= all_num) {
			for (let i = 0; i < answer_arr.length; i++) {
				let match = answer_arr[i].match(/第(\d+)题([\s\S]*)/);
				if (!match)
					continue;
				let index = match[1];
				let user_answer = match[2].toUpperCase().replace(/(\s+)|(\.*)/g, '');
				if ((user_answer.length < 1) || (!activity_answer[index]))
					continue;
				for (let j = 0; j < activity_answer[index].length; j++) {
					let answer = activity_answer[index][j].toUpperCase().replace(/(\s+)|(\.*)/g, '');
			...
			let user_answer1 = content.toUpperCase().replace(/(\s+)|(\.*)/g, '');
			for (let key in activity_answer) {
				for (let k = 0; k < activity_answer[key].length; k++) {
					let answer1 = activity_answer[key][k].toUpperCase().replace(/(\s+)|(\.*)/g, '');
